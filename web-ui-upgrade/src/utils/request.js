...
service.interceptors.request.use(config => {
    config.headers[CommonConsts.TOKEN_KEY] = getToken() || '';
...
service.interceptors.response.use(
    ...
    res => {
        ...
        if (code !== 200) {
            // 弹框信息
            Notification.error({title: '错误', message: msg, position: 'bottom-right', duration: 2500});
            // 返回错误
            return Promise.reject(new Error(msg));
        }
        // 如果是200,则返回数据
        Notification.success({title: '完成', message: msg, position: 'bottom-right', duration: 1500});
        return res.data;
    },
    error => {
