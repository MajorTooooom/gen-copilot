...
public class PackageBeforeTests {
    ...
    private static void distToStatic() {
        // 当前项目的根目录
    ...
    private static void distToNginx() {
        // 当前项目的根目录
        String currentDir = SystemUtil.getUserInfo().getCurrentDir();
        // 前端工程的dist目录
        ...
        // Nginx
        File toDir = FileUtil.file(currentDir, "attachments", ".docker", "data", "nginx", "dist");
        // 删除目录下除了`.gitkeep`的所有旧文件
        FileUtil.loopFiles(FileUtil.file(toDir)).stream().forEach(file -> {
            if (!file.getName().equals(".gitkeep")) {
                FileUtil.del(file);
            }
        });
        // 目录拷贝
        FileUtil.copyContent(FileUtil.file(dist), FileUtil.file(toDir), true);
    }
}
